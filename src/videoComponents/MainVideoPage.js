...
// Helper: attach and try to play muted first
function attachAndPlay(videoEl, mediaStream) {
  if (!videoEl || !mediaStream) return;
...
export default function MainVideoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 🔴 Peer disconnect → reset signaling so we can create a new offer later
  ...
  useEffect(() => {
    ...
    // 🔄 allow re-negotiation (no early return on currentRemoteDescription)
    if (pc.signalingState !== 'have-local-offer') return;
